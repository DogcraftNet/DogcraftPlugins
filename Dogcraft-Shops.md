...
**Min-price floor.** A discount can never drop a single bundle's price below the configured `min-price` (default `0.01`). The floor is enforced by **clamping**, not rejection — if the math would land below `min-price`, the discounted portion is silently raised to `min-price` and the sale proceeds at that price. The buyer pays at least `min-price` per bundle regardless of how steep the code is; bundles 2..N on per-player codes always pay full price. Owners can therefore create codes at any percent up to `discount.max-percent` and trust the system to honor the floor at sale time without surprise rejections.
