...
- Purchasing the same effect type again **extends the duration** - the new time is added onto whatever is remaining (e.g. 30s left + buying 5m = 5m30s total)
...
Multiple buffs can share the same `PotionEffect` (e.g. different durations of Haste). Only one effect of each type is active at a time. Purchasing the same effect type again appends the new duration onto the remaining time rather than replacing it.
...
1. **Redis (primary)** - Active buffs use TTL keys. Config uses a versioned hash. Opt-outs use Set + Pub/Sub.
...
| `EffectUpdater` | Repeating task that syncs from Redis and applies active effects to players. |
| `RedisManager` | Lettuce Redis client wrapper. Handles connections, TTL keys, hashes, sets, and Pub/Sub. |
| `GUI` | InventoryGui-based purchase menu with pagination and enchantment glow on active buffs. |
| `NPC` | Spawns and validates the configurable buff vendor entity. |
...
|-----------|---------|---------|
| [Paper API](https://papermc.io) | 1.20.1+ | Server API |
| [DogcraftEconomy](https://repo.dogcraft.net) | 1.0.3+ | Currency system |
| [InventoryGui](https://repo.minebench.de) | 1.4.1 | GUI framework |
| [Lettuce](https://lettuce.io) | 6.3.2 | Redis client |

InventoryGui and Lettuce are shaded into the plugin jar. Paper and DogcraftEconomy must be present on the server.
